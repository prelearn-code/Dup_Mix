...
from scripts.paper_tests.benchmarks import bench_b_prove, save_result


def main() -> None:
    parser = argparse.ArgumentParser(description="Metric B: prove benchmark")
    ...
    result = bench_b_prove(challenge_counts, args.repeats, args.n_blocks, args.dup_ratio, args.private_ratio)
    paths = save_result(result)
    print(f"[B] done. csv={paths['csv']} json={paths['json']}", flush=True)
